...
async function ensureBotUserId(ctx: Parameters<PluginModule['plugin_init']>[0], event?: OB11Message): Promise<string> {
  if (BOT_USER_ID) {
    return BOT_USER_ID;
  }
...
export const plugin_onmessage: PluginModule['plugin_onmessage'] = async (ctx, event) => {
  ...
  if (event.post_type && event.post_type !== EventType.MESSAGE) return;
